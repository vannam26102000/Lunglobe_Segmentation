...
def postrocessing(label_image, spare=[]):
    ...
    regionmask = skimage.measure.label(label_image) #gan nhan cho cac vung cua mot mang so nguyen
    # print(regionmask)
    origlabels = np.unique(label_image) # tra ve cac phan tu duy nhat da duoc sap xep
    # print(origlabels)
    origlabels_maxsub = np.zeros((max(origlabels) + 1,), dtype=np.uint32)  # will hold the largest component for a label
    # print(origlabels_maxsub)
    regions = skimage.measure.regionprops(regionmask, label_image) # do cac thuoc tinh cua cac vung duoc gan nhan
    # print(regions)
    regions.sort(key=lambda x: x.area) # sap xep cac vung theo thu tu tang dan
    # print(regions)
    ...
    # print(region_to_lobemap[regionmask])
